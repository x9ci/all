...
class TextExtractor:
    """استخراج وتحليل النصوص من كتب الشطرنج PDF"""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        # تكوين المسافات والحدود
        self.x_tolerance = 3
        self.y_tolerance = 3
    ...
    def _init_chess_patterns(self):
        ...
        self.chess_patterns = {
            # نقلات القطع
            'piece_moves': re.compile(r'\b([KQRBN][a-h]?[1-8]?x?[a-h][1-8])\b'),
            # نقلات البيادق
            ...
            # التقييمات
            'evaluations': re.compile(r'[+\-]?[0-9]*\.?[0-9]+'),
            # الرموز التقييمية
            'symbols': re.compile(r'[!?]{1,2}|[\u2686\u2687\u2688\u2689]|[⩱⩲±∓∞⌓⟳↑↓⇆]'),
            # علامات الشاه
            'checks': re.compile(r'[+#]$'),
            # نتائج المباراة
            'results': re.compile(r'(?:1-0|0-1|½-½|1/2-1/2)')
        }
        
        # الكلمات المحجوزة التي لا تترجم
        self.chess_keywords = {
            # قطع الشطرنج الإنجليزية
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for page_num, page in enumerate(pdf.pages, 1):
                    self.logger.info(f"استخراج النص من الصفحة {page_num}")
    ...
    def process_page(self, page) -> Dict:
        ...
        try:
            ...
            # تحليل مكونات الصفحة
            blocks = self._analyze_page_components(words, page)
            ...
            page_info = {
                ...
                'metadata': {
                    ...
                    'has_diagrams': bool(self._extract_chess_diagrams(page))
                }
            }
    ...
    def _analyze_page_components(self, words: List[Dict], page) -> List[Dict]:
        """تحليل مكونات الصفحة وتصنيفها"""
        blocks = []
        current_block = []
        current_y = None
    ...
    def _create_content_block(self, words: List[Dict]) -> Optional[Dict]:
        """إنشاء كتلة محتوى مع تحديد نوعها"""
        if not words:
            return None
        ...
        block = {
            ...
            'type': block_type,
            'needs_translation': block_type == 'regular',
    ...
    def _analyze_text_content(self, text: str) -> Tuple[str, Dict]:
        ...
        for pattern_name, pattern in self.chess_patterns.items():
            if matches := pattern.finditer(text):
                content_info['has_chess_moves'] = True
                content_info['chess_elements'].extend(
                    [m.group() for m in matches]
                )
                return 'chess', content_info
                
        # التحقق من وجود رموز شطرنج
        if any(char in text for char in '♔♕♖♗♘♙♚♛♜♝♞♟'):
            content_info['has_chess_moves'] = True
            return 'chess', content_info
            
        return 'regular', content_info

    def _extract_chess_diagrams(self, page) -> List[Dict]:
        """استخراج رسوم الشطرنج التوضيحية"""
        diagrams = []
        try:
            # البحث عن الرسوم التوضيحية باستخدام الجداول
            tables = page.extract_tables()
            for table in tables:
                if self._is_chess_diagram(table):
                    diagrams.append({
                        'type': 'chess_diagram',
                        'content': table,
            ...
            self.logger.error(f"خطأ في استخراج رسوم الشطرنج: {str(e)}")
        return diagrams

    def _is_chess_diagram(self, table: List[List]) -> bool:
        """التحقق مما إذا كان الجدول يمثل رسماً للشطرنج"""
        if not table or len(table) != 8:
            return False
        # التحقق من وجود رموز الشطرنج في الجدول
        chess_symbols = set('♔♕♖♗♘♙♚♛♜♝♞♟')
        return any(
            any(cell and any(char in chess_symbols for char in cell) 
                for cell in row)
            for row in table
        )

    def _get_table_bbox(self, table: List[List]) -> Tuple[float, float, float, float]:
        """الحصول على إحداثيات الجدول"""
        # تنفيذ منطق استخراج الإحداثيات
        return (0, 0, 0, 0)  # مؤقتاً
...
class PDFProcessor:
    ...
    def __init__(self, config_manager: ConfigManager, cache_manager: CacheManager):
        ...
        self.text_extractor = TextExtractor()  # إضافة TextExtractor
    ...
    def process_pdf(self, input_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            extracted_pages = self.text_extractor.extract_from_pdf(input_path)
            self.stats['total_pages'] = len(extracted_pages)

            # معالجة كل صفحة
            processed_pages = []
            for page_num, page_content in enumerate(tqdm(extracted_pages, desc="تقدم المعالجة"), 1):
                self.logger.info(f"معالجة صفحة {page_num}")
                
                # معالجة وترجمة المحتوى
                processed_page = self._process_page(page_content)
    ...
    def _process_page(self, page_content: Dict) -> Dict:
        """معالجة صفحة واحدة"""
        processed_page = page_content.copy()
        processed_blocks = []
        ...
        for block in page_content['blocks']:
            try:
                ...
                if block['type'] == 'chess':
                    # حفظ نقلات الشطرنج كما هي
                    self.stats['chess_moves'] += len(block.get('metadata', {}).get('chess_elements', []))
                    processed_blocks.append(block)
                    
                elif block['needs_translation']:
                    # ترجمة النص العادي
                    translated_block = self._translate_block(block)
        ...
        processed_page['blocks'] = processed_blocks
    ...
    def _translate_block(self, block: Dict) -> Dict:
        ...
        try:
            translated_text = self.translation_processor.translate_text(original_text)
            if translated_text and translated_text != original_text:
                ...
                translated_block['translated_text'] = original_text
        ...
        try:
            ...
            for page in pages:
                ...
                if page.get('diagrams'):
                    self._draw_diagrams(c, page['diagrams'])
    ...
    def _draw_diagrams(self, canvas_obj, diagrams: List[Dict]):
        """رسم الرسوم التوضيحية للشطرنج"""
        try:
            for diagram in diagrams:
                if diagram['type'] == 'chess_diagram':
                    self._draw_chess_diagram(canvas_obj, diagram)
        except Exception as e:
            self.logger.error(f"خطأ في رسم الرسوم التوضيحية: {str(e)}")

    def _draw_chess_diagram(self, canvas_obj, diagram: Dict):
        """رسم رقعة شطرنج"""
        # سيتم تنفيذ هذه الميزة في تحديث لاحق
        pass
